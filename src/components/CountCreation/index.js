...
const CountCreation = ({close}) => {
    ...
    const checks = () => {
        const error = counts.some((count) => {
            return count.uuid.toUpperCase() === uuid.toUpperCase() || count.name.toUpperCase() === name.toUpperCase();
        })
        if (error) {
        ...
        <div className={"overlay"}>
            <div className={"countCreate overlay__inner"}>
                <form onSubmit={handleSubmit}>
                    ...
                    {!success &&
                    <div className="countCreate__submit--error">{errorText}</div>}
                    <div className="countCreate__submit">
                        <input type={"submit"} value={"Add new count"} disabled={!success}/>
                        &nbsp;&nbsp;&nbsp;
                        <button value={"cancel"} onClick={cancelBtn}>Cancel</button>
                    </div>
                </form>
            </div>
        </div>
